...
# ==========================================================
# 🎯 육하원칙(5W1H) 및 기승전결 강제 AI 기사 생성 엔진
# ==========================================================
def generate_ai_article(category_name):
    # 1. 실시간 구글 뉴스 피드로부터 현재 실제 속보 헤드라인 확보
...
"""
    ...
    CATEGORY_DIRECTIVES = {
        "스포츠": (
            ...
            "경기 전후반의 결정적 승부처, 선수의 구체적인 활약상, 감독의 전술적 승부수를 육하원칙에 맞추어 생생하게 중계하듯 전달하세요. "
            "추상적인 미사여구(극적인 승부, 에이스의 폭발 등)만 늘어놓는 것을 엄격히 금지합니다."
        ),
        "AI/테크": (
        ...
        "경제/주식": (
            "실제 시장을 흔드는 특정 테마 업종과 대표 관련 종목명, 코스피/코스닥 지수 흐름, 한국은행/미 연준 금리, 환율 수치 등 "
        ...
        "연예계뉴스": (
            "실제 화제가 되고 있는 스타의 이름, 방송 프로그램명, 영화/드라마 작품명, 차트 순위나 흥행 지표 등 "
            "구체적 사실을 바탕으로 대중문화계의 최신 트렌드와 팬덤 반응을 품격 있게 조명하세요."
        ),
        "지역창": (
            "강원도 속초 및 영동 지역을 중심으로, 실제 지명, 축제명, 문화재 명칭, 로컬 명소와 맛집의 구체적 특징, "
            "지역 주민과 관광객에게 유익한 방문 정보(교통, 일정, 추천 코스)를 생생하고 정확하게 취재하듯 서술하세요."
        ),
        "세상이야기": (
            "실제 사건/사고 현장 또는 감동적인 미담의 주인공, 발생 시점과 장소, 구체적인 선행 및 사건 경위를 육하원칙에 따라 따뜻하고 설득력 있게 전달하세요."
        ),
        "건강/복지": (
            "보건복지부, 질병관리청 등 공신력 있는 기관의 최신 지침, 질환의 정확한 의학적 명칭, 정부 복지 혜택의 구체적인 신청 자격 요건과 소득 기준액을 명확히 밝히세요."
        ),
        "생활정보": (
            "정부 지원금, 부동산 절세 체크리스트, 에너지 절약법 등 일상에서 즉시 활용할 수 있는 법령 기준, 세액 감면 수치, 신청 기간 및 사이트를 꼼꼼하게 명시하세요."
        )
    }

    directive = CATEGORY_DIRECTIVES.get(category_name, "정확한 팩트에 기반한 정론 기사를 작성하세요.")

    # 육하원칙 + 기승전결 구조를 강제하는 엄격한 저널리즘 프롬프트
    journalism_directive = f"""
당신은 대한민국 최고 정론지의 20년 차 베테랑 사회부·체육부 수석 기자입니다.
기사는 반드시 **[육하원칙(5W1H)]**과 **[기승전결(起承轉結)]** 구조를 철저하게 준수해야 합니다.

[작성 절대 원칙]:
1. **첫 번째 줄**: 따옴표나 특수문자 없이, 독자의 시선을 사로잡는 핵심 팩트 기반의 [기사 제목]을 한 줄로만 작성하세요.
2. **두 번째 줄**: 반드시 빈 줄로 남겨 두세요.
3. **세 번째 줄 (첫 문단 - 기: 起 / 종합 리드문)**: 
   - 기사 서두에서 **[누가, 언제, 어디서, 무엇을, 어떻게, 왜]** 사건이 일어났는지 핵심 팩트를 두괄식으로 완벽히 요약하세요. (모호한 표현 절대 금지, 고유명사 필수)
4. **본문 전개 (승·전·결 구조화)**:
   - 본문은 총 4개 문단 이상으로 작성하고, 각 단락 위에는 반드시 '### 소제목'을 붙이세요.
   - **### [승: 承] 사건의 구체적 경위와 현장 전개**: 경기의 구체적 득점 상황/사건의 발생 전말/기술의 구체적 작동 방식 서술.
   - **### [전: 轉] 핵심 쟁점 및 전문가 분석·반응**: 당사자의 공식 반응, 이해관계자의 대립 또는 승부처에 대한 정밀 분석.
   - **### [결: 結] 향후 파급 효과 및 관전 포인트**: 향후 일정, 후속 경기 전망, 독자 및 사회에 미칠 영향과 시사점.
5. **금지 사항**:
   - 팀명이나 선수명 없이 '어느 한 팀이 극적으로 이겼다'처럼 두루뭉술하게 얼버무리는 허위·소설식 서술은 절대 금지합니다.
   - 본문 첫머리에 제목을 반복하지 마세요. 표준적인 한국어 격식 보도체(~다)로 명료하게 작성하세요.
...
def admin_studio(request: Request, admin_auth: str = Cookie(None)):
    ...
    <body>
        ...
        <div class="box" style="border-top: 5px solid #27ae60;">
            <h3>🤖 1. 상단: AI 자동 기사 발행 (육하원칙 5W1H 및 기승전결 탑재)</h3>
            <form action="/admin/create-auto" method="post">
                ...
                <button type="submit">🚀 육하원칙 기반 실시간 정밀 기사 발행</button>
            </form>
        </div>
...
def create_ai_expand(
    ...
    system_directive = (
        ...
        "1. 서론에서 육하원칙(누가, 언제, 어디서, 무엇을, 어떻게, 왜)을 분명히 서술하고, 최소 4개 이상의 문단으로 기승전결을 갖추어 작성하세요.\n"
